...
const OUT = 'C:\\留存\\Game Now\\soul-blade-plus\\文档\\血刃-M1.3-测试截图';
...
async function menuTo(page, idx) { // M1.3: 五项菜单 (0=STORY 1=VSCPU 2=LOCALVS 3=TRAINING 4=HOWTO)
...
/* M1.3 鼠标: 画布逻辑坐标 -> CSS 坐标换算后真实点击 */
async function mclick(page, lx, ly) {
  const box = await page.evaluate(() => {
    const r = document.getElementById('game').getBoundingClientRect();
    return { left: r.left, top: r.top, width: r.width, height: r.height };
  });
  await page.mouse.click(box.left + lx * box.width / 1024, box.top + ly * box.height / 576);
  await sleep(180);
}
...
(async () => {
  ...
  try {
    ...
    for (let i = 0; i < 5; i++) await tap(page, 'KeyS');
    note((await st(page, 'G.titleSel')) === 0, '标题菜单五项循环导航', 'S×5 回到 0');
    ...
    note((await st(page, 'ROSTER.length === 7')) === true, '花名册 7 人(含貂蝉)', '');
    await sleep(300);
    ...
    await tap(page, 'KeyA'); await tap(page, 'KeyA'); // wrap 0 → 6(diaochan) → 5(angela)
    note((await st(page, "ROSTER[G.select.cursor] === 'angela'")) === true, '选人 A 键回绕(七人制)到 angela', '');
    await tap(page, 'KeyJ');
    ...
    await tap(page, 'Escape', 45, 420);
    await ensureTitle(page);
    /* ============ M1.3: STORY 闯关 · 纯鼠标导航进入 + 真实战斗流程 ============ */
    curStep = '21-闯关鼠标导航';
    await mclick(page, 512, 355);   // STORY 菜单条
    await waitState(page, "G.screen === 'select' && G.select.quest === true", '鼠标进入闯关选人');
    note(true, '鼠标点击 STORY 菜单', '');
    // 点击貂蝉格(第7格) -> 难度
    const tile7 = await st(page, "(() => { const N = ROSTER.length, TS = 104, GAP = 14, x0 = (1024 - (N * TS + (N - 1) * GAP)) / 2; return Math.round(x0 + 6 * (TS + GAP) + TS / 2); })()");
    await sleep(250);
    await shot(page, '21-选人-七人网格');
    await mclick(page, tile7, 136);
    await waitState(page, "G.select.phase === 'diff' && G.select.p1 === 'diaochan'", '鼠标选貂蝉→难度');
    note(true, '鼠标点击选人网格(貂蝉)', '');
    await mclick(page, 262, 295);   // EASY 卡(闯关验收用低难度保通率)
    await waitState(page, "G.screen === 'quest'", '闯关启动', 9000);
    note((await st(page, "Quest.st && Quest.st.phase === 'talk'")) === true, '闯关开幕剧情对话', '');
    await sleep(400);
    await shot(page, '22-闯关-开幕对话');
    // 对话点击推进
    for (let i = 0; i < 6; i++) {
      if ((await st(page, "Quest.st.phase === 'talk'")) !== true) break;
      await mclick(page, 512, 480);
    }
    await waitState(page, "Quest.st.phase === 'walk'", '对话后进入走图');
    note(true, '对话条鼠标推进', '');
    await page.keyboard.down('KeyD');
    await waitState(page, "Quest.st.phase === 'fight'", '第一波触发', 15000);
    await page.keyboard.up('KeyD');
    note((await st(page, 'Quest.st.enemies.filter(e => !e.dead).length >= 2')) === true, '第一波双敌同屏', 'en=' + (await st(page, 'Quest.st.enemies.length')));
    await sleep(500);
    await shot(page, '23-闯关-第一波混战');
    // 真实清波: 走近最近敌人连打
    curStep = '24-闯关战斗';
    const qFight = async (budgetMs) => {
      const t0 = Date.now();
      let i = 0;
      while (Date.now() - t0 < budgetMs) {
        const ph = await st(page, 'Quest.st.phase');
        if (ph === 'over') { // 阵亡→原生重试流(点击) —— 重试也是被验收的路径
          await mclick(page, 512, 300);
          await waitState(page, "Quest.st.phase === 'talk'", 'retry talk', 5000);
          for (let k = 0; k < 6; k++) {
            if ((await st(page, "Quest.st.phase === 'talk'")) !== true) break;
            await mclick(page, 512, 480);
          }
          await page.keyboard.down('KeyD'); // 走回触发点
          await waitState(page, "Quest.st.phase === 'fight' || Quest.st.phase === 'bossfight'", 're-engage', 20000).catch(() => {});
          await page.keyboard.up('KeyD');
          continue;
        }
        if (ph !== 'fight' && ph !== 'bossfight') return ph;
        const gap = await st(page, "(() => { const p = Quest.st.player, es = Quest.st.enemies.filter(e => !e.dead); if (!es.length) return 0; es.sort((a, b) => Math.abs(a.x - p.x) - Math.abs(b.x - p.x)); return es[0].x - p.x; })()");
        const dir = gap >= 0 ? 'KeyD' : 'KeyA';
        if (Math.abs(gap) > 115) {
          await page.keyboard.down(dir);
          await sleep(Math.min(500, Math.abs(gap) * 3));
          await page.keyboard.up(dir);
        }
        await tap(page, i % 3 === 2 ? 'KeyK' : 'KeyJ', 35, 90);
        if (i % 8 === 7) await tap(page, 'KeyI', 35, 50);
        i++;
      }
      return 'timeout';
    };
    let ph1 = await qFight(60000);
    note(ph1 === 'walk', '第一波真实清空→走图', 'phase=' + ph1);
    // 走到第二波并清空
    await page.keyboard.down('KeyD');
    await waitState(page, "Quest.st.phase === 'fight'", '第二波触发', 20000).catch(() => {});
    await page.keyboard.up('KeyD');
    ph1 = await qFight(80000);
    note(ph1 === 'walk' || ph1 === 'talk', '第二波真实清空', 'phase=' + ph1); // talk=已达 Boss 前哨
    // ---- 通用推进驱动器: 任意相(走图/波次/Boss对白/Boss战/阵亡重试/过关对白)
    //      持续按 phase 分派推进, 直到进入第二幕。真实击破 Boss 是唯一通路。
    let shot24 = false, shot25 = false, sawBoss = false;
    const t0b = Date.now();
    while (Date.now() - t0b < 330000) {
      if ((await st(page, 'Quest.st.level')) === 1) break;
      const phN = await st(page, 'Quest.st.phase');
      if (phN === 'talk') {
        const isBossTalk = (await st(page, "Quest.st.talkNext === 'bossfight'")) === true;
        if (isBossTalk && !shot24) { await sleep(300); await shot(page, '24-闯关-BOSS对白'); shot24 = true; }
        await mclick(page, 512, 480);
        await sleep(250);
      } else if (phN === 'bossfight') {
        if (!sawBoss) {
          sawBoss = (await st(page, 'Quest.st.boss && Quest.st.boss.isBoss === true')) === true;
          if (!shot25) { await sleep(400); await shot(page, '25-闯关-BOSS战'); shot25 = true; }
        }
        await qFight(120000);
      } else if (phN === 'fight' || phN === 'over') {
        await qFight(120000);
      } else { // walk
        await page.keyboard.down('KeyD');
        await waitState(page, "Quest.st.phase !== 'walk'", '走图推进', 25000).catch(() => {});
        await page.keyboard.up('KeyD');
      }
    }
    note(sawBoss, 'BOSS 登场(血条见截图)', '');
    note((await st(page, 'Quest.st.level')) === 1, 'BOSS 真实击破→通关第一幕→第二幕', 'kills=' + (await st(page, 'Quest.st.kills')));
    await sleep(400);
    await shot(page, '26-闯关-第二幕开幕');
    // 退出闯关(ESC 暂停 -> ESC 退出), 回标题
    await tap(page, 'Escape', 45, 350);
    await tap(page, 'Escape', 45, 420);
    await waitState(page, "G.screen === 'title'", '闯关退出回标题');
    note((await st(page, 'STAGE.left === 60 && STAGE.right === 964')) === true, '退出后 STAGE 边界还原', '');
    ...
    /* ============ M1.3 quick pages: 貂蝉超杀 + 青铜神殿舞台 ============ */
    curStep = '27-diaochan超杀';
    p = await newGamePage(browser, '?fight&training&p1=diaochan&p2=mack&ff=100');
    await waitState(p, "typeof G !== 'undefined' && G.screen === 'fight' && G.phase === 'fight'", 'training diaochan', 60000);
    const sup4 = await fireSuper(p, '27-超杀-diaochan花舞');
    note(sup4.seen && sup4.dmg > 0, 'diaochan 花舞乱影(三连拍)', 'dmg=' + sup4.dmg);
    await p.close();

    curStep = '28-青铜神殿';
    p = await newGamePage(browser, '?fight&p1=diaochan&p2=wukong&ai=easy&stage2=3&ff=140');
    await waitState(p, "typeof G !== 'undefined' && G.screen === 'fight' && G.phase === 'fight'", 'hall stage fight', 60000);
    note((await st(p, 'G.stageSel === 3')) === true, '青铜神殿舞台路由', '');
    await p1Burst(p, 2600);
    await shot(p, '28-战斗-青铜神殿-diaochan');
    note(true, '貂蝉 vs 悟空 @青铜神殿 实战', '');
    await p.close();
  } catch (e) {
