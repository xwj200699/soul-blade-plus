# -*- coding: utf-8 -*-
"""Build 刀魂PLUS-M1-优化版.html (single file).

M1 P0 fixes vs the first single-file build:
  - ui-lab embeds the lossless .webp set ONLY (the engine requests
    '/assets/ui-lab/*.webp'); .png originals stay on disk as source, NOT embedded
  - debug-only variants excluded (title-kanban/gunsen/torii, announce-brush-slim)
  - shim implements resolveEmbeddedAsset(): strips origin + leading '/',
    query/hash, resolves webp<->png both ways
  - unresolved assets/ requests are blocked (console.warn + reject), never
    hitting the network — file:// stays fully offline & silent
"""
import base64
import json
import mimetypes
...
OUT = os.path.join(ROOT, "发布", "刀魂PLUS-M1.1-修复版.html")

mimetypes.add_type("font/ttf", ".ttf")
mimetypes.add_type("font/woff2", ".woff2")
mimetypes.add_type("image/webp", ".webp")

SKIP = {  # debug variants (reachable only via ?te= / ?ann= URL params)
    "assets/ui-lab/title-kanban.png", "assets/ui-lab/title-gunsen.png",
    "assets/ui-lab/title-torii.png", "assets/ui-lab/announce-brush-slim.png",
}
...
for dirpath, _, files in os.walk(os.path.join(ROOT, "assets")):
    for fn in files:
        ...
        if key in SKIP:
            dropped += 1
            continue
        if key.startswith("assets/ui-lab/") and key.endswith(".png"):
            skipped_png += 1  # webp twin is embedded instead
            continue
        assets[key] = data_url(p)
print(f"embedded: {len(assets)} · ui-lab png kept-on-disk-not-embedded: {skipped_png} · debug variants dropped: {dropped}")
...
<head>
<meta charset="utf-8">
<title>刀魂 SOUL BLADE PLUS · M1.1 修复版</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<link rel="icon" type="image/png" href="{assets.get('assets/favicon.png', '')}">
<style>
{css2}
</style>
</head>
