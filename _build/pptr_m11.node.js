...
const HTML = 'C:\\留存\\Game Now\\soul-blade-plus\\发布\\刀魂PLUS-M1.1-修复版.html';
const OUT = 'C:\\留存\\Game Now\\soul-blade-plus\\文档\\刀魂PLUS-M1.1-测试截图';
...
/* trigger a super (training mode = infinite P1 meter) and screenshot the CINE
   itself: wait for superSeq (set on contact), let the banner/hitstop pass,
   then shoot mid-sequence with 2+ hits already landed. */
async function fireSuper(page, shotName) {
  let seen = false;
  for (let attempt = 0; attempt < 3 && !seen; attempt++) {
    await approach(page, attempt === 0 ? 130 : 100, 5000);
    await tap(page, 'KeyI', 40, 60);
    const t0 = Date.now();
  ...
  if (seen) {
    // banner freeze is ~28 ticks; then wait until a couple of cine hits are in
    const t1 = Date.now();
    while (Date.now() - t1 < 2500) {
      const done = await st(page, 'G.fighters[0].superSeq ? G.fighters[0].superSeq.done : 99');
      if (done === 99 || done >= 2) break; // mid-spectacle (or already finished)
      await sleep(50);
    }
  }
...
(async () => {
  ...
  try {
    ...
    const sup1 = await fireSuper(page, '12-训练-angela超杀');
    note(sup1.seen, 'angela 超杀触发(火焰法阵)', 'dummy lost ' + sup1.dmg + ' hp');
    ...
    const sup2 = await fireSuper(p, '19-超杀-wukong');
    note(sup2.seen && sup2.dmg > 0, 'wukong 三段棍法超杀', 'dmg=' + sup2.dmg);
    await p.close();
    ...
    note(sup3.seen && sup3.dmg > 0, 'houyi 射日箭雨超杀', 'dmg=' + sup3.dmg);
    await p.close();
  } catch (e) {
